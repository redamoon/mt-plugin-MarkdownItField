package MarkdownItField::ContentFieldType::MarkdownItField;
use strict;
use warnings;
...
sub data_load_handler {
    ...
    my @bodies = $app->multi_param(
        'markdown-it-field-' . $field_data->{id} . '-body');
    my @previews = $app->multi_param(
        'markdown-it-field-' . $field_data->{id} . '-preview');
